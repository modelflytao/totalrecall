...
- **synth 保护 applied patterns(关键修复)**:Phase 1 的 `synth` 会合并/删除近似 Pattern。若它把一个
  applied pattern 合并掉,会**丢失 `applied_at`/`applied_rule`/粘性状态 并使 id 失稳**,直接破坏闭环。
  故 `synth.run` 必须**把有 `applied_at` 的 pattern 排除出合并候选**(它们不参与 `_ask_merges`,既不被
  合并掉、也不当 survivor)。这同时保证了上面 pin 的 id 稳定。
...
active ──propose+apply──> (applied_at 记录, 仍 active/待验证)
  ...
  └── 久未出现 ──> fading        ineffective ──(MVP: 终态, 手动处理)──> 见下
```
- **MVP 中 `ineffective` 是终态**:它是给你的**信号**("这条规则没用")。再处理走手动——你直接编辑受管
  `totalrecall-rules.md` 里该 pattern 的规则块(就在 `<!-- pattern: id -->` 下),或删掉它。
  **自动"再起草更好的规则"留作后续**(涉及重置提案/覆盖已写规则块,见 §12 不做)。
- proposer 的候选筛选据此只取 `status == active && 无 applied_at`(不含 ineffective);
  避免与"终态 ineffective"矛盾。
...
- **@import 路径约束**:注入的是 `@<rules 文件名>`(basename),Claude Code 按 CLAUDE.md 所在目录解析。
  默认 `rules_file` 与 `claude_md` 同在 `~/.claude/`,正确;若你把两者配到不同目录,需改为相对/绝对路径(MVP 默认安全,文档注明此约束)。
- **synth 不碰 applied**:`synth` 合并候选排除 applied patterns(见 §4.3),故合并不会丢失闭环字段或改 applied id。
...
- ✅ **synth 保护 applied patterns**(排除出合并候选,保 id 与闭环字段)
...
- ❌ **`ineffective` 自动再起草/再应用**(MVP 终态,手动改受管规则块;自动再提案涉及重置提案+覆盖已写规则块,后续做)
...
1. **闭环依赖归并可靠性**:复发必须归并回同一 pattern 才能判 ineffective/resolved。靠"pin applied patterns 进 catalog"
   + Phase 1 的 pattern_id 复用 + **synth 不再删/合并 applied pattern(§4.3 修复)** 共同保证 id 稳定;
   仍需评估分析 skill 的复用命中率。若复发被新建成别的 pattern,会漏判(假 resolved)。
